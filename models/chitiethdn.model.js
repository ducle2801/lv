...
const DetailImportInvoice = function (detailImportInvoice) {
  ...
  this.id_th = detailImportInvoice.idms;
  this.id_lsp = detailImportInvoice.idkt;
...
DetailImportInvoice.sumNumber = (idsp, result) => {
  mysql.query(
    `SELECT SUM(so_luong_nhap) as so_luong_nhap FROM chi_tiet_hdn WHERE id_sp='${idsp}'`,
    (err, res) => {
      if (err) {
        console.log("ERROR: ", err);
        result(err, null);
...
DetailImportInvoice.sumNumberProduct = (result) => {
  mysql.query(
    "SELECT SUM(so_luong_hdn) as tong_so_luong FROM chi_tiet_hdn",
    (err, res) => {
      if (err) {
        console.log("ERROR: ", err);
        result(err, null);
...
DetailImportInvoice.getDetailProductList = (idhdn, result) => {
  mysql.query(
    `SELECT * FROM chi_tiet_hdn INNER JOIN san_pham ON chi_tiet_hdn.id_sp = san_pham.id_sp INNER JOIN thuong_hieu ON chi_tiet_hdn.id_th = thuong_hieu.id_th INNER JOIN loai_san_pham ON chi_tiet_hdn.id_lsp = loai_san_pham.id_lsp WHERE chi_tiet_hdn.id_hdn='${idhdn}'`,
    (err, res) => {
      if (err) {
        console.log("ERROR: ", err);
        result(err, null);
